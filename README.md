...
The examples show the default values for `ALPINE_VERSION=3.24.1 EDITION=std`, `ARCH` defaults to the OS architecture `uname -m`.
...
```
make mkimage ALPINE_VERSION=3.24.1
```
...
```
make iso ALPINE_VERSION=3.24.1 EDITION=std
```

### Run the ISO with qemu

```
make run ALPINE_VERSION=3.24.1 EDITION=std
```

### Run the ISO with Lima

```
make lima ALPINE_VERSION=3.24.1 EDITION=std
```
